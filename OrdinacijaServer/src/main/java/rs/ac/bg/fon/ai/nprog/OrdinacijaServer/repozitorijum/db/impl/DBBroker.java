...
/**
 * Predstavlja broker za pristup bazi podataka koji implementira interfejs
 * IRepozitorijum.
 * 
 * Omogucava izvrsavanje osnovnih operacija nad objektima tipa
 * OpstaDomenskaKlasa
 *
 * @author Iva Stanisic
 */
	...
	public boolean obrisi(OpstaDomenskaKlasa param) throws Exception {
		try {
			...
			if (param.vratiSlozenPrimarniKljuc() == null) {
				sql = "DELETE FROM " + param.imeTabele() + " WHERE " + param.vratiNazivPrimarnogKljuca() + " = "
		...
		try {
			...
			if (param.vratiSlozenPrimarniKljuc() == null) {
				sql = "UPDATE " + param.imeTabele() + " SET " + param.uslovZaIzmenu() + " WHERE "
						+ param.vratiNazivPrimarnogKljuca() + " = " + param.vratiPrimarniKljuc();
			} else {
				sql = "UPDATE " + param.imeTabele() + " SET " + param.uslovZaIzmenu() + " WHERE "
