...
class SOKreirajPlanIshraneTest {
	...
	void testizvrsiSpecificnuOperaciju() throws Exception {
		...
		assertEquals(sdf.format(plan.getDatum()),
				sdf.format(((PlanIshrane) ucitajPlan.vratiListu().get(poslednji)).getDatum()));
		assertEquals(plan.getNutricionista(),
				((PlanIshrane) ucitajPlan.vratiListu().get(poslednji)).getNutricionista());
	...
	@Test
	void testizvrsiSpecificnuOperacijuPlanBezStavki() throws Exception {
		ucitajPacijente.izvrsiOperaciju(new Pacijent());
		assertNotNull(ucitajPacijente.getLista());
		assertTrue(ucitajPacijente.getLista().size() > 0);
		ucitajNutricioniste.izvrsiOperaciju(new Nutricionista());
		...
		PlanIshrane plan = new PlanIshrane();
		plan.setBrojDana(10);
		plan.setDatum(new Date());
		plan.setNutricionista((Nutricionista) ucitajNutricioniste.getLista().get(0));
		plan.setPacijent((Pacijent) ucitajPacijente.getLista().get(0));
		assertThrows(Exception.class, () -> kreirajPlan.izvrsiOperaciju(plan));
	}

	@Test
	@DisplayName("Kreiranje plana ishrane koji vec postoji")
	void testizvrsiSpecificnuOperacijuKreirajPlanPostoji() throws Exception {
		ucitajPlan.izvrsiOperaciju(new PlanIshrane());
		assertNotNull(ucitajPlan.vratiListu());
		assertTrue(ucitajPlan.vratiListu().size() > 0);
		PlanIshrane plan = (PlanIshrane) ucitajPlan.vratiListu().get(0);
		assertThrows(Exception.class, () -> kreirajPlan.izvrsiOperaciju(plan));
	}

	@Test
	void testKreirajPlanIshraneNullObjekat() {
		assertThrows(Exception.class, () -> kreirajPlan.izvrsiOperaciju(null));
	}

	@Test
	void testKreirajPlanIshraneLosaKlasa() {
